...
def test_cli_run_with_program_path(tmp_path: Path):
    ...
    original_run = __import__(
        "autojepa.controller.continuous", fromlist=["run_continuous"]
    ).run_continuous
    ...
    with patch("autojepa.cli.run_continuous", side_effect=spy_run_continuous):
        runner = CliRunner()
