...
> **NOTE:** Entries dated <= 2026-05-14 are inherited verbatim from `autoresearch-rl`
> (this repo's `git remote upstream`). Entries dated 2026-05-15 onwards are
> AutoJEPA-specific. Per the user's NEVER LIE rule, only validated outcomes
> appear here — no aspirational claims.
...
`/home/epappas/workspace/spacejar/autoresearch-rl`) specialized for JEPA
pretraining per architecture writeup
https://gist.github.com/epappas/2567a53350ba0d6ca064c71986a76046 (draft v0.1).

### Added

- `src/autojepa/` — package carry-over from `src/autoresearch_rl/` (65 files), module name renamed
- `tests/` — test suite carry-over (52 files), import paths renamed
- `pyproject.toml` — new project identity (`autojepa` 0.1.0); **`basilica-sdk` promoted to default dependency** (Basilica is the prime deployment target); new `[jepa]` extra for torch/lightning/torchmetrics/transformers/webdataset
- `README.md`, `CLAUDE.md`, `Makefile` — rewritten for AutoJEPA identity, Basilica-first
- `TODO.md` — phased roadmap mirroring writeup
- `docs/phase-0-baseline.md` — verified Phase 0 baseline with test counts
- `docs/research/` — citable research corpus (ingestion in progress)
- `git remote upstream` — `../autoresearch-rl` (cherry-pick reference only, capped ~1h/wk)

### Verified evidence (Phase 0)
...
The 9 failing tests reference `examples/<name>/prepare.py` fixtures from
the upstream `examples/` tree which was deliberately not carried over (115 MB of
model artifacts). They will be re-pointed at `examples/ijepa-cifar10/` in Phase 2.

### Not done

- No JEPA-specific code (eval/, masking/, models/, gates.py) — Phase 1
- No `examples/` directory — Phases 2-3
- No campaign runs against Basilica yet
